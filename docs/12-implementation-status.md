...
- โมเดลการ post (OPEN-{ปี} ลงวันที่ 31 ธ.ค. ปีก่อน + MOVE-{ปี} ลงวันที่ 31 ธ.ค. ปีนั้น) **รองรับนำเข้าหลายปีงบ/บริษัทพร้อมกันแล้ว** (Phase 2, 2026-06-20): JournalEntry มีคอลัมน์ `FiscalYear` (explicit) — ยอดต่อปีกรองด้วย FiscalYear ผ่าน `FsJournalNets` ไม่เดาจากวันที่อีก จึงไม่เบิ้ลแม้ OPEN-(Y+1) ลงวันที่ชนกับ MOVE-Y. verify บ.242 (2 ปี: 2025+2026): TB/GL/BS/AdjTB ต่อปีถูกต้อง, BS สมดุล diff=0 ทั้ง 2 ปี, GL closing==TB endNet 96/96, ไม่มี double-count, แก้ bug TB/GL "ยอดยกมา=0". **carry-forward AJE ข้ามปี (Option B) — เสร็จแล้ว (2026-06-20):** (1) งบ BS/PL/Notes/Equity **รวม AdjustmentEntry (AJE ปิดงบใน-ระบบ)** แล้ว = งบที่ finalize เป็นงบหลังปรับปรุง (ผ่าน `FsJournalNets.CumulativeWithAdjustmentsAsync`). (2) เมื่อ Report Package ปี (Y-1) Final/Locked → สร้าง `CF-{Y}` (JournalEntry SourceModule=CarryForwardOpening) = ผลรวม AJE ปี finalized < Y, map บัญชี P&L→RE, เป็นยอดยกมาปี Y. verify บ.242: AJE 2025 (Dr ค่าใช้จ่าย/Cr เงินสด 1000) → BS/PL 2025 หลังปรับปรุง −1000 สมดุล; finalize → CF-2026 = Dr RE/Cr เงินสด สมดุล; BS 2026 −1000 สมดุล, **netProfit 2026 ไม่เปลี่ยน**, AdjustedTB begin รวม CF; idempotent + cleanup กลับ baseline. ข้อจำกัด: opening คำนวณจาก AJE ใน-ระบบ (Express brought-forward = base); ปีที่ Locked ไม่ถูกแตะ (เตือน stale)
...
4. ~~(Phase 2) ออกแบบ posting ใหม่รองรับนำเข้าหลายปีงบพร้อมกัน~~ **เสร็จแล้ว** (2026-06-20, JournalEntry.FiscalYear — ดูข้อจำกัดที่ทราบ)
...
- **ใบกรอก ภ.พ.30 (e-Filing) — เสร็จ (2026-06-14):** ใบช่วยกรอกค่าลงหน้า "ข้อมูลการคำนวณภาษี" ของเว็บ RD (ดู reference/vat/pp30_input.jpg) — เลือกเดือน → แสดงค่าตรงช่อง/ลำดับเดียวกับฟอร์มยื่นจริง: ยอดขายในเดือนนี้ (=outputBase+outputZeroRated) / ยอดขายอัตรา0 / ยอดขายยกเว้น (0 — ISVAT ไม่เก็บ) / ยอดซื้อมีสิทธิ์ (inputBase) / ภาษีขาย (outputVat) / ภาษีซื้อ (inputVat) / ภาษีเดือนนี้รอคำนวณ (=ขาย−ซื้อ) / ภาษีชำระเกินยกมา (กรอกเอง — ระบบไม่เก็บยอดสะสม) / ภาษีสุทธิรอคำนวณ. **frontend-only** (reuse useVatReport ไม่แตะ backend) + ExportMenu (พิมพ์/บันทึก). - **ไฟล์โอนย้าย ภ.พ.30 (.txt) — เสร็จ (2026-06-14):** ปุ่ม "⬇ ไฟล์โอนย้าย (.txt)" ในแท็บใบกรอก → อัปโหลดหน้า e-Filing "โอนย้ายข้อมูล ภ.พ.30 (ยื่นรวมกัน)" (ดู reference/vat/pp30_txt.jpg). `IVatPp30TransferExportService`/`VatPp30TransferExportService` (Infrastructure) สร้าง **TIS-620 delimited** (default pipe, มี header) 1 สาขา = 1 แถว, คอลัมน์: สาขาที่/ยอดขายในเดือนนี้/อัตรา0/ยกเว้น/ยอดซื้อมีสิทธิ/ภาษีขาย/ภาษีซื้อ (RD แมพคอลัมน์ที่ step2 + เลือก delimiter/header เอง; เดือน/สาขา/ประเภทยื่น กรอกบนฟอร์มเว็บ). `GetVatPp30TransferQuery(company,year,month,delimiter,includeHeader)` reuse GetVatReportQuery; `GET /vat/pp30-transfer`. **verify:** pipe+header → header ไทย TIS-620 ถูก + แถว `00000|78584.20|0.00|0.00|56995.85|5500.88|3989.71`; comma+ไม่มี header ก็ถูก; ปุ่มดาวน์โหลด UI ยิง API 200 ไม่มี console error
- **หมายเหตุ:** layout ไฟล์อิงช่อง "ข้อมูลการคำนวณภาษี" — เพราะ RD ให้แมพคอลัมน์เอง ลำดับไม่ตายตัว (ไม่เหมือน ภ.ง.ด.); ถ้า RD step2 ต้องการคอลัมน์ต่างจากนี้ ปรับ Headers/cols ใน service ได้ทันที
- **หลายสาขา (ยื่นรวมกัน) — เสร็จ (2026-06-14):** ISVAT มีฟิลด์ `DEPCOD` (รหัสสาขา เช่น HO00/BR01) → เพิ่ม `VatEntry.DepartmentCode` + import map (migration `AddVatEntryDepartmentCode`). `GetVatPp30BranchesQuery` แตกยอดต่อสาขา (group DEPCOD; **blank+HO\* → สำนักงานใหญ่ 00000**, อื่น ๆ ใช้เลขท้ายรหัส BR01→00001) → `GET /vat/pp30-branches`; ไฟล์โอนย้ายเปลี่ยนเป็น **1 แถว/สาขา**. UI: ตาราง "แยกตามสาขา" โผล่เมื่อ isMultiBranch (ใบกรอกด้านบนยังเป็นยอดรวมทุกสาขา = ถูกสำหรับแบบยื่นรวม). **Verify (DEELUX20 id287, ธ.ค.2025):** 2 สาขา — 00000(สนญ.+blank) ขาย 52,796.00/ภาษีซื้อ 94.08, 00001(BR01) ขาย 378,592.17/ภาษีขาย 26,291.82/ภาษีซื้อ 16,063.66 ตรง ISVAT เป๊ะ; ไฟล์โอนย้าย 2 แถว TIS-620; รวม = ใบกรอก 431,388.17; UI ผ่าน Preview ไม่มี console error
- **mapping เลขสาขา + resync — เสร็จ (2026-06-14):** `VatBranchMapping` (per company: DepartmentCode→RdBranchNo+IsHeadOffice+BranchName, unique, migration `AddVatBranchMapping`) override กฎอัตโนมัติ. `GetVatBranchMappingsQuery` (list DEPCOD ที่พบ + mapping/convention) · `UpsertVatBranchMappingCommand` (validate เลขสาขาเป็นตัวเลข) · `DeleteVatBranchMappingCommand` (คืนค่ากฎ) · `GET/PUT/DELETE /vat/branch-mappings`. **Resync:** `ResyncVatDepartmentsCommand` + `POST /vat/resync-departments` — ดึง ISVAT ใหม่ทุกบริษัท **VAT-only** (รัน VatEntryImporter ต่อบริษัท, ไม่แตะ GL/posting/snapshot) เติม DEPCOD ให้ข้อมูลเดิม. Frontend: `Pp30BranchMappingModal` (ปุ่ม "แมพเลขสาขา" บนแท็บ) — ตาราง DEPCOD แก้เลขสาขา/สนญ. + บันทึก/คืนค่า + ปุ่ม resync. **Verify:** list 3 DEPCOD (blank/HO00/BR01) convention default; override BR01→00003 → branches เปลี่ยนทันที; 422 เลขไม่ใช่ตัวเลข; reset กลับ 00001; resync 3 บริษัท/3,964 รายการ/multiBranch 1; UI modal ผ่าน Preview ไม่มี console error
- **หมายเหตุเลขสาขา:** ถ้าไม่ได้แมพใช้กฎ (HO/ว่าง→00000, BR##→0####); แมพ override ได้ต่อบริษัท. **ต้อง resync 1 ครั้ง** หลังอัปเกรด เพื่อเติม DEPCOD ให้ข้อมูลที่นำเข้าก่อนหน้า
...
- **หมายเหตุ/อนาคต:** ผลขาดทุนสะสมติดตามแบบยอดรวม (ยังไม่แยกอายุ 5 ปีรายปี)
- **ใบ ภ.ง.ด.50 (PDF ฟอร์มราชการ) เฟส A — เสร็จ (2026-06-14):** ฟอร์มจริง `reference/vat/CIT50.pdf` เป็น **AcroForm 649 ฟิลด์** → ใช้เป็น template + **overlay ข้อความตามพิกัดฟิลด์ด้วย PdfSharpCore (MIT) + ฟอนต์ไทย Tahoma** (spike พบว่า fill field ตรง ๆ ข้อความไทยไม่ขึ้น จึง overlay). ทำ **หน้า 1** (เลขผู้เสียภาษี comb 13 ช่อง / ชื่อบริษัท / รอบบัญชี วัน-เดือน-ปี พ.ศ.) + **หน้า 2 การคำนวณภาษี** (Text6=ฐานภาษี, Text7=ภาษีที่คำนวณได้, Text10=ภาษีหัก ณ ที่จ่าย, Text14=รวมหัก, Text15/17=คงเหลือ/รวมสุทธิ) จาก TAX engine. `IPnd50PdfService`/`Pnd50PdfService` (Infra, font resolver + พิกัด hardcode) + `GetPnd50PdfQuery` (reuse GetTaxComputationQuery + รอบบัญชีจาก FiscalYearStartMonth) + `GET /corporate-tax/pnd50-pdf` + ปุ่ม "⬇ ภ.ง.ด.50 (PDF)" ใน Pnd50Page. template = `Datacenter.Infrastructure/Resources/CIT50.pdf` (CopyToOutputDirectory). **Verify (JSP CONNX 226/2025):** PDF 8 หน้า valid, ค่าลงถูกช่อง (ฐานภาษี 510,160.48 / ภาษี 31,524.07 / คงเหลือ 31,524.07), ชื่อไทย+อังกฤษ render ผ่าน PdfSharpCore+Tahoma, เลขผู้เสียภาษีเรียง comb, วันที่รอบบัญชี 01/01/2568–31/12/2568; ปุ่ม UI ยิง 200.
- **ที่ตั้งสำนักงาน (เพิ่ม 2026-06-14):** ตาม reference/vat/PP50_2018.pdf หน้า 1 ต้องมีที่อยู่แยกช่อง → เพิ่ม **ที่อยู่แยกช่องบน ClientCompany** (`AddrBuilding/RoomNo/Floor/Village/HouseNo/Moo/Soi/Road/SubDistrict/District/Province` + ใช้ `PostalCode` เดิม; migration `AddClientCompanyStructuredAddress`) แทน parse flat ทุกครั้ง (แก้ต่อช่องได้ + field-level audit). เติมอัตโนมัติจาก `Address` ด้วย `ThaiAddressParser` ตอน import (ถ้าว่าง ไม่ทับที่แก้เอง) + backfill ครั้งเดียวใน DbInitializer; แก้ที่ฟอร์มลูกค้า (`<details>` ที่อยู่แยกช่อง). ภ.ง.ด.50 อ่าน structured ก่อน (fallback parse). เลขผู้เสียภาษีเรียงตรงช่อง (วัด raster grid). **Verify:** 226 backfill 39/6/หมู่12/นาป่า/เมืองชลบุรี/ชลบุรี/20000; แก้ road→persist→PDF แสดง; ฟอร์มโหลดค่าครบ ไม่มี console error
- **ประกอบกิจการ + ISIC + checkbox (เพิ่ม 2026-06-14):** เพิ่ม `BusinessActivity`/`IsicCode` บน ClientCompany (ป้อนเอง, Express ไม่มี; migration `AddClientCompanyAuditorIsic`) → ภ.ง.ด.50 field 23 (ประกอบกิจการ) + field 24 (ISIC comb 6); checkbox หน้า 1 ☑ ยื่นปกติ (Group1) + ☑ สถานะนิติบุคคลไทย (Group2, ข้ามถ้าชื่อมี "มหาชน") — วาดเครื่องหมายถูก (vector ✓ ผ่าน XPen+DrawLines ไม่ใช่ "X"); รอบบัญชีเป็น comb (วัน2/เดือน2/ปี4) วาดทีละช่อง
- **ผู้ลงนามรับผิดชอบงบต่อรอบปี — ผู้สอบบัญชี + ผู้ทำบัญชี (refactor 2026-06-14):** ทั้งผู้สอบบัญชีและผู้ทำบัญชีเปลี่ยนได้รายปี → ย้ายออกจาก ClientCompany มาเป็น entity ใหม่ **`CompanyAuditor`** key unique (ClientCompanyId, FiscalYear): ผู้สอบ `AuditorName`/`AuditorLicenseNo`/**`AuditorTaxId`(13)** + ผู้ทำบัญชี `BookkeeperName`/**`BookkeeperTaxId`(13)** + `SignDate`/`Note` (migration `AddCompanyAuditorPerYear` drop AuditorName/AuditorLicenseNo เดิมบน ClientCompany + สร้างตาราง; `AddCompanyAuditorBookkeeper` เพิ่ม 2 ช่องผู้ทำบัญชี; audit whitelist). `GetCompanyAuditorQuery` (Exists=false ถ้าว่าง) + `SaveCompanyAuditorCommand` (upsert; **ทั้งชื่อผู้สอบและผู้ทำบัญชีว่าง=ลบของปีนั้น**; validator taxId ผู้สอบ/ผู้ทำบัญชี=13 หลัก) + `GET/PUT /corporate-tax/auditor`. ภ.ง.ด.50 หน้า 1 เติมบล็อกผู้สอบ field 43(taxId comb13 กลุ่ม 1-4-5-2-1 วัดจาก raster grid)/44(ชื่อ)/45(ทะเบียน comb8) + **บล็อกผู้ทำบัญชี field 50(taxId comb13 กริดเดียวกับ 43)/51(ชื่อ)** + **วันที่ในรายงานผู้สอบ field 46(วัน comb2)/47(เดือน comb2)/48(พ.ศ. comb4) จาก SignDate** ตามผู้ลงนามของปีที่ยื่น. Frontend: การ์ด "ผู้ลงนามรับผิดชอบงบ (รอบปี)" ใน `Pnd50Page` 2 ส่วน (ผู้สอบ/ผู้ทำบัญชี) + ลบช่องผู้สอบออกจากฟอร์มลูกค้า (เหลือ hint). **Verify (226/2024):** save/get/PDF=200; field 43/50 เรียงตรงช่องทุกหลัก (`1×13`/`2×13`) + ชื่อทั้งสองลงถูกช่อง + วันที่รายงาน 30/05/2568 เรียงตรง comb; การ์ด UI 2 ส่วนครบ ไม่มี console error; รีเซ็ตข้อมูลทดสอบแล้ว
- **โปรไฟล์สำนักงานบัญชี + สำนักงานสอบบัญชี (2026-06-14):** ผู้ใช้ระบบเป็นสำนักงานบัญชี → เพิ่ม **`OfficeProfile`** (singleton ค่ากลาง: OfficeName/TaxId/BranchCode/Address/Phone; migration `AddOfficeProfileAndAuditFirm`; audit whitelist) สำหรับ "สำนักงานทำบัญชี" (f52) ใช้ทุกบริษัทอัตโนมัติ — `GetOfficeProfileQuery`/`SaveOfficeProfileCommand` (upsert singleton, ไม่ผูก IRequireCompanyAccess) + `GET/PUT /office-profile` + หน้า `features/settings/pages/OfficeProfilePage` (เมนูระบบ → โปรไฟล์สำนักงานบัญชี). **สำนักงานสอบบัญชี (f49)** = สังกัดผู้สอบ (ต่างลูกค้า/ปีได้) → เพิ่ม `AuditFirmName`/`AuditFirmTaxId` ใน CompanyAuditor (รายปี) + ช่องในการ์ดผู้ลงนาม. PDF เติม field 49 (สำนักงานสอบ) จาก auditor.AuditFirmTaxId + field 52 (สำนักงานทำบัญชี) จาก OfficeProfile.TaxId — comb 13 กริดฝั่งขวา (วัด raster grid). **Verify (226/2024):** office save/get=200; auditFirm+office → PDF f49=`8×13`/f52=`9×13` เรียงตรง comb; หน้าโปรไฟล์ + การ์ดผู้ลงนาม (ช่องสำนักงานสอบ) render ครบ ไม่มี console error; รีเซ็ตข้อมูลทดสอบแล้ว
- **แก้ comb 13 ช่องเลขผู้เสียภาษีเยื้อง (fix 2026-06-14):** f43/f50 (ฝั่งซ้าย) + f49/f52 (ฝั่งขวา) เลขไม่ตรงช่อง (คลาด ~2.4pt). สาเหตุ: เดิมวัดแค่เส้นแบ่ง**กลุ่ม**(ทึบ) แล้วหารช่องเท่า ๆ กันในกลุ่ม — แต่ cell ในกลุ่มไม่ได้กว้างเท่ากันเป๊ะ + ขอบกลุ่มใช้ midpoint ของเส้นคู่ทำให้เพี้ยน. แก้: ตรวจจับ**เส้นแบ่งทุกเส้น**(รวมเส้นประย่อย, threshold คอลัมน์ดำ >18% สูง) → ได้ 18 dividers ตัดช่องว่างระหว่างกลุ่ม (gap <8pt) เหลือ 13 cell → center=กลางคู่ divider จริง. ค่าใหม่ Auditor/Bookkeeper(f43/f50)=[43.5,60.8,72.4,84.0,95.7,113.0,124.6,136.2,147.9,159.5,177.1,188.7,206.1] · Firm(f49/f52)=[402.7,420.0,431.6,443.2,454.9,472.2,483.8,495.4,507.1,518.7,536.3,547.9,565.3]. เลขบริษัทด้านบน (f1 TaxIdCellCenters) วัดซ้ำตรงของเดิม ไม่แตะ. **Verify:** เลข `1234567890123` ทั้ง 4 ช่อง แต่ละหลักอยู่กลาง cell ของตัวเอง (gotcha: เดิมเทสต์ด้วยเลขซ้ำ 8888.. บังการเยื้อง — ต้องเทสต์เลขไม่ซ้ำ)
- **ทะเบียนผู้ลงนาม master + ค่าเริ่มต้นบริษัท + override รายปี (2026-06-14):** ผู้สอบ/ผู้ทำบัญชีใช้ซ้ำหลายลูกค้า → ทำเป็น master. entity ใหม่ **`Auditor`** (Name/Type CPA-TA/LicenseNo/TaxId/AuditFirmName/AuditFirmTaxId/IsActive) + **`Bookkeeper`** (Name/TaxId/IsActive); migration `AddAuditorBookkeeperMaster` (+ FK `ClientCompany.DefaultAuditorId/DefaultBookkeeperId` = ผู้ลงนามประจำบริษัท + `CompanyAuditor.AuditorId/BookkeeperId` = override รายปี; เก็บ free-text เดิมไว้ legacy + backfill ใน DbInitializer → สร้าง master + ตั้ง company default; audit whitelist Auditor/Bookkeeper). **Resolution (PDF):** ผู้ลงนามของปี = override รายปี (CompanyAuditor) **??** ค่าเริ่มต้นบริษัท (master) → ปีที่ไม่เปลี่ยนใช้ default อัตโนมัติ ไม่ต้องกรอกซ้ำ. **override แล้วเลื่อนเป็น default อัตโนมัติ (2026-06-14):** SaveCompanyYearSigners เมื่อ override ผู้สอบ/ผู้ทำบัญชี จะตั้ง ClientCompany.Default ตามไปด้วย → ปีถัดไปดึงคนใหม่; ปีเก่าที่ override ไว้ freeze ค่าเดิม (มี AuditorId ของตัวเอง) รักษาประวัติ. hook invalidate signers ทั้งบริษัท. verify: override 2027→temp#2 ⇒ default=2, 2028 resolved=2, 2025 ยัง=1 (freeze). **หน้าภาพรวมมอบหมายผู้ลงนาม (2026-06-14):** `GetSignerAssignmentsQuery` (ทุกบริษัทที่เข้าถึงได้ผ่าน ICompanyAccessGuard + filter search/auditorId/bookkeeperId) → ผู้สอบ/ผู้ทำบัญชีประจำ + จำนวนปี override; `GET /corporate-tax/signer-assignments`. Frontend `settings/pages/SignerAssignmentsPage` (เมนูระบบ → มอบหมายผู้ลงนาม) — ตารางทุกบริษัท แก้ผู้สอบ/ผู้ทำบัญชีประจำ inline (dropdown master) + กรองตามผู้สอบ/ผู้ทำบัญชี (ดูว่าใครดูแลบริษัทใดบ้าง) + batch save แถวที่แก้ (เรียก SetCompanyDefaultSigners ต่อบริษัท). แก้รายบริษัท/override รายปี ยังทำที่หน้า ภ.ง.ด.50 ได้. verify: 71 บริษัท, filter auditorId=1 → 1 บริษัท, dirty highlight + ปุ่มบันทึก(N). CRUD master = `GET/POST/PUT/DELETE /auditors` `/bookkeepers` (ค่ากลาง ไม่ผูก company; soft-delete IsActive=false) + `GET /corporate-tax/signers` / `PUT /corporate-tax/signers/default` / `PUT /corporate-tax/signers/year`. Frontend: หน้าทะเบียน `settings/pages/AuditorsPage`+`BookkeepersPage` (เมนูระบบ) + การ์ดในหน้า ภ.ง.ด.50 = dropdown เลือกผู้สอบ/ผู้ทำบัญชี (จาก master) + วันที่รายงาน + ปุ่มเดียว "บันทึกผู้ลงนาม" (ตัด checkbox override/ส่วนแยก default ออก — เลือกใหม่บันทึก = overwrite ปีนี้ + เลื่อนเป็น default ปีถัดไป ผ่าน SaveCompanyYearSigners) + เตือนกฎอิสระ (ผู้สอบ ≠ ผู้ทำบัญชี เลขผู้เสียภาษีเดียวกัน) + เตือน TA. **Verify:** backfill ย้าย 226/2025 (สุธาทิพย์ ระโส + ศักดา จันทมาตร) → Auditor#1/Bookkeeper#1 + ตั้ง company default; signers 226/2024 (ไม่มี override) → resolved = default ✓; PDF 2025 แสดงชื่อจาก master; หน้าทะเบียน + การ์ด dropdown render ครบ ไม่มี console error
- **checkbox หน้า 2 (เงินได้ที่ต้องเสียภาษี + การคำนวณภาษี) เสร็จ (2026-06-14):** ติ๊กตามผล TAX engine — รายการ 2 Group4: (1) กำไรสุทธิ ถ้า AdjustedProfit≥0 / (2) ขาดทุนสุทธิ ถ้า<0; การคำนวณภาษี: RateScheme=SmeTiered → Group5 (1) กรณีลดอัตราภาษี + Group6 (1.2) "ทุนชำระแล้ว ≤ 5 ล้าน" (=SME, อ่านจากคำอธิบายท้ายหน้า), Flat20/Custom ไม่ติ๊ก; Group7 คงเหลือ + Group8 รวมภาษี: ชำระเพิ่มเติม ถ้า NetPayable≥0 / ชำระไว้เกิน ถ้า<0. radio วาด vector ✓ (DrawCheck) ที่ widget rect; เพิ่ม RateScheme+IsNetProfit ใน Pnd50FormData. verify 226/2025 (SME+กำไร+ภาษีต้องชำระ): ☑กำไรสุทธิ ☑ลดอัตรา ☑(1.2) ☑ชำระเพิ่มเติม×2 ตรงช่อง
- **หน้า 3 (รายการที่ 3 reconciliation) เสร็จ (2026-06-14):** เติมบรรทัด 1-16 + 21 คอลัมน์ 2(เสียภาษี)+3(รวม) [คอลัมน์ 1 ยกเว้น เว้น = non-BOI]. ดึง breakdown จาก `GetProfitLossQuery` (Revenue=TotalIncome, Cogs, GrossProfit, Sga=TotalExpenses−Cogs+|Finance|) + TAX engine (NetProfitBeforeTax→L9, AddBack→L11, Deduction→L13, AdjustedProfit→L14, LossUsed→L15, NetTaxableIncome→L16/L21). reconcile: L9=ProfitBeforeTax, L16=L21=NetTaxableIncome (ตรงหน้า 2). **แยกบรรทัด 1/4 (2026-06-14, เทียบแบบยื่นจริง TAX_FORM_*.pdf):** L1 รายได้โดยตรง = I1+I2 (ขาย+บริการ), L4 รายได้อื่น = I3+I4 (ที่เหลือ), L3 gross = operating−COGS, L5 = L3+L4 (เดิมรวมทุกอย่างใน L1 ผิดโครงสร้าง). gitignore `reference/vat/TAX_FORM_P500003056971.pdf` (แบบยื่นจริง มีข้อมูลบริษัท PDPA). checkbox Group9(L3 กำไร/ขาดทุนขั้นต้น)/Group10(L9 กำไร/ขาดทุนสุทธิ)/Group11(L21). field map: col2 x359 col3 x466.9, y per line (anchor radio Group9 y143.8=L3, Group10 y247.1=L9). `Pnd50Page3Data` ใน Pnd50FormData. verify 226/2025: 5,117,439.91→ต้นทุน 3,159,763.18→ขั้นต้น 1,957,676.73→สุทธิ 510,160.48→เงินได้สุทธิ 510,160.48 (reconcile + ตรงหน้า 2) ✓
- **หน้า 7 (รายการที่ 12 งบดุล) เสร็จ (2026-06-14) — crosswalk RefCode→CIT50:** เติมครบทุกบรรทัด 140-161 ผ่าน **crosswalk fixed (บรรทัด CIT50 ← RefCode ผังงบ, ใช้ทุกบริษัทไม่ต้องมี UI)** จาก `GetBalanceSheetQuery` (ยอด presentation ต่อ RefCode). map: 140←A1, 141←A7, 142←A3, 143←A2+A4+TXR, 144←A8, 145←A5, 146←A9+A10, 148←A6, 149←L3, 150←L1, 151←L5, 152←L2+TXP, 153←L6, 154←L4, 156←C1, 158/159←RE (+checkbox Group12 กำไร/ขาดทุนสะสม ตามเครื่องหมาย RE), รวมสินทรัพย์/หนี้สิน/ทุน/รวม ← Total*. `Pnd50Page7Data`. verify 226/2025: ทุก RefCode ลงบรรทัดครบ, รวมสินทรัพย์=รวมหนี้สิน+ทุน=2,821,772.93 (สมดุล ตรง BS API), ☑ ขาดทุนสะสม (RE −744,833.65). field x459 w100.6.
- **ระบบ map บัญชี→บรรทัด CIT50 + รายการที่ 8 เสร็จ (2026-06-14):** entity **`Cit50ScheduleLine`** (taxonomy: Code/ScheduleNo/Label/SortOrder + **เก็บพิกัด PDF ในตัว** PdfPage/X/Y/W + IsCatchAll/IsTotal) + **`AccountCit50Mapping`** (ต่อบริษัท: AccountCode→Cit50LineCode; เหมือน AccountStatementMapping; audit whitelist). seed รายการ 8 (16 บรรทัด: พนักงาน/กรรมการ/ค่าเช่า/ค่ารับรอง/ภาษีธุรกิจเฉพาะ/ต้นทุนการเงิน/ทำบัญชี/สอบบัญชี/ค่าธรรมเนียม/หนี้สูญ/ค่าเสื่อม/อื่นๆ catch-all/รวม) ใน DbInitializer (idempotent); migration `AddCit50AccountMapping`. CQRS `GetCit50MappingQuery`(บัญชี X1+X2+X3 จาก P&L + mapping ปัจจุบัน + lines) + `SaveCit50MappingCommand` + `GET/PUT /corporate-tax/cit50-mapping`. **PDF generic schedule cells** (`Pnd50ScheduleCell` Page/X/Y/W/Amount) — GetPnd50PdfQuery aggregate ยอดบัญชีตาม mapping (**บัญชีที่ไม่ map → catch-all "รายจ่ายอื่น" อัตโนมัติ → total reconcile เสมอ**), เติม 2 คอลัมน์ (เสียภาษี x−108.3 + รวม). Frontend `Cit50MappingPage` (route `/pnd50/cit50-mapping` + ลิงก์ในหน้า ภ.ง.ด.50): ตารางบัญชี + dropdown เลือกบรรทัด + batch save. verify 226/2025: 40 บัญชี, map 9 (ค่าเสื่อม/ค่าเช่า/สอบบัญชี/ดอกเบี้ย) → รายการ 8 ลงถูกบรรทัด, รวม 1,447,516.25 = รายการ 3 บรรทัด 8 (reconcile), 2 คอลัมน์ครบ; รีเซ็ต mapping ทดสอบแล้ว
- **เฟส B ที่เหลือ:** รายการ 5/7/9/10 (ต้นทุนผลิต/รายจ่ายอื่น/รายจ่ายต้องห้าม/รายได้ยกเว้น) — extend ได้โดย seed Cit50ScheduleLine เพิ่ม + เพิ่มแหล่งบัญชี (รายการ 9 ต้องดึงจาก add-back ที่แยกประเภท ซึ่งยังเก็บยอดรวม); รายการ 4 (สินค้าต้น-ปลาย) + 11 (ขาดทุน 5 ปีรายปี) ต้องข้อมูลเพิ่ม; หน้า 8 = คำรับรอง/ลายเซ็น (มือ); checkbox BOI หน้า 2 Group3. cleanup: ลบ free-text legacy บน CompanyAuditor หลัง backfill ทุก env

### ✅ Task workflow (งานทั่วไป + workboard ข้ามบริษัท) — เสร็จ (2026-06-20, Phase 2)
งานนอกเหนือภาษี 6 ประเภท (Compliance) — มอบหมาย/ติดตามต่อบริษัท + ภาพรวมข้ามทุกบริษัท

- **Domain:** `WorkTask` (Title/Description/Category/Status/Priority/DueDate/AssignedUserId/CompletedAt+By, BaseEntity) + enum `WorkTaskStatus`(Open/InProgress/Done/Cancelled) `WorkTaskPriority`(Low/Normal/High) int. migration `AddWorkTasks`. เพิ่มใน AuditableEntityRegistry (field-level audit)
- **CQRS (`Features/Tasks/`):** Create/Update/Delete (IRequireCompanyAccess) + UpdateStatus (set CompletedAt/By เมื่อ Done) + Assign + `GetWorkTasksQuery` (ต่อบริษัท) + **`GetWorkboardQuery`** (ข้ามบริษัท: scope ด้วย ICompanyAccessGuard.GetAccessibleCompanyIds, **union WorkTask + ComplianceTask** → WorkItemDto, filter openOnly/assignee/dueBefore/includeCompliance) + `GetAssignableUsersQuery` (Admin + CompanyUserAccess). action-level + field-level audit ครบ
- **API:** `TasksController` `/api/v1/work-tasks` (GET/board/assignable-users · POST · PUT · PATCH status/assign · DELETE)
- **Frontend:** `features/tasks` (mirror adjustments) — `/tasks` 2 แท็บ: "งานของบริษัท" (CRUD + เปลี่ยนสถานะ/ผู้รับผิดชอบ inline + TaskFormModal) / "workboard" (รวมทุกบริษัท + filter ของฉัน/ยังไม่เสร็จ/รวมภาษี/ครบกำหนด + badge ประเภท). เมนู "ภาพรวม → งาน / มอบหมายงาน"
- **Dashboard widget "งานของฉัน" (2026-06-20):** `features/dashboard/components/MyTasksWidget` บนหน้า Dashboard (แสดงเสมอ) — งานค้าง/เกินกำหนดที่มอบหมายให้ผู้ใช้ปัจจุบัน รวมทุกบริษัท (reuse GetWorkboardQuery `assignedUserId=user.userId&openOnly`) + ลิงก์ /tasks. **frontend-only** (ไม่แตะ backend). verify บ.242: ค้าง 2/เกินกำหนด 1 แสดงถูก ไม่มี console error
- **ครบวงจร (2026-06-20):** (1) **Recurring** — `WorkTask.RecurrenceType`(None/Daily/Weekly/Monthly/Yearly)+`Interval`; ปิดงาน Done + ตั้งซ้ำ → spawn งานถัดไป Open อัตโนมัติ (เลื่อน DueDate ตามรอบ, checklist รีเซ็ต, ไม่ loop/ไม่ double) — ไม่ต้องมี scheduler. (2) **Subtask/checklist** — entity `WorkTaskItem` (cascade) + จัดการใน Create/Update (replace-all) + `PATCH /work-tasks/{id}/items/{itemId}` toggle; แสดง progress x/y + expand ติ๊กในตาราง. (3) **Email reminder** — `SendTaskRemindersCommand` (Admin) รวมงาน open overdue/≤daysAhead → digest 1 ฉบับ/ผู้รับผิดชอบ (reuse IEmailSender; +`User.Email`); `POST /work-tasks/send-reminders` + ปุ่ม workboard; **opt-in `DailyTaskReminderService`** (BackgroundService วันละครั้ง, default OFF จนกว่าตั้ง `TaskReminders:Enabled=true`+SMTP). migration `AddWorkTaskRecurrenceItemsUserEmail`. verify e2e: recurring spawn/no-double, checklist toggle 1/3, reminder skip(ไม่มีอีเมล)/fail(SMTP ไม่ตั้งค่า — recipient resolve ถูก), UI ผ่าน Preview (badge ประจำ+checklist+ปุ่มส่งเตือน), build/tsc/vite 0 error
- **สิทธิ:** ทุกคนที่มีสิทธิในบริษัทจัดการได้ (universal edit + audit, ไม่มี approval — ตาม AGENTS.md)
- **Verify (บ.242 + 224):** create/list/status→Done(CompletedAt set)/assign/delete; workboard union 95 รายการ (WorkTask + compliance, Source ถูก, เกินกำหนดสีแดง); filter openOnly/dueBefore; field-audit จับ Status/CompletedBy; UI 2 แท็บ + สร้างงานผ่าน modal ผ่าน Preview ไม่มี console error; test data ลบแล้ว
- **ขอบเขต/อนาคต:** ไม่มี recurring/subtask/discussion (Note เดี่ยว); pull-based (ไม่มี push/email reminder); compliance อ่านอย่างเดียวใน board
