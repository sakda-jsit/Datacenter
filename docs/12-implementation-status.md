...
- **ผู้ตรวจสอบและรับรองบัญชีต่อรอบปี (refactor 2026-06-14):** ผู้สอบบัญชีเปลี่ยนได้รายปี → ย้ายออกจาก ClientCompany มาเป็น entity ใหม่ **`CompanyAuditor`** key unique (ClientCompanyId, FiscalYear): `AuditorName` / `AuditorLicenseNo` / **`AuditorTaxId` (13 หลัก)** / `SignDate` / `Note` (migration `AddCompanyAuditorPerYear` — drop AuditorName/AuditorLicenseNo เดิมบน ClientCompany + สร้างตารางใหม่; audit whitelist). `GetCompanyAuditorQuery` (คืนค่าว่าง Exists=false ถ้ายังไม่บันทึก) + `SaveCompanyAuditorCommand` (upsert; ชื่อว่าง=ลบของปีนั้น; validator taxId=13 หลัก) + `GET/PUT /corporate-tax/auditor`. ภ.ง.ด.50 หน้า 1 เติม field 44 (ชื่อ) / field 45 (ทะเบียน comb 8) / **field 43 (เลขผู้เสียภาษีผู้สอบ comb 13 — กลุ่ม 1-4-5-2-1 วัดจาก raster grid)** ตามผู้สอบของปีที่ยื่น. Frontend: การ์ด "ผู้ตรวจสอบและรับรองบัญชี (รอบปี)" ใน `Pnd50Page` (แยกตามปี) + ลบช่องผู้สอบออกจากฟอร์มลูกค้า (เหลือ hint ชี้ไปหน้า ภ.ง.ด.50). **Verify (226/2024):** save/get/PDF = 200; field 43 = `1\|2345\|67890\|12\|3` เรียงตรงช่องทุกหลัก; การ์ด UI render ครบ 4 ช่อง ไม่มี console error; รีเซ็ตข้อมูลทดสอบแล้ว
- **เฟส B (ยังไม่ทำ):** หน้า 3-7 รายละเอียดงบกำไรขาดทุน/งบดุล/ปรับปรุงกำไร (map ทีละช่อง — ใหญ่, ซ้ำงบการเงิน); ที่อยู่ผู้สอบบัญชี/วันที่ในรายงาน (field 46-49) + ลายเซ็น/วันที่ยื่น (เซ็นมือ)
